...
(define RECORD-STATE
  (make-ws "playloop" 0.0 0.5 0)) ;record the loop
(define PLAY-STATE
  (make-ws "1" 0.0 0.5 0)) ;keyLastPressed is 1 so not similar to initial state, plays loop
...
(define stopKey kick)
(define playloop ding)
(define stoploop ding)
...
(define (checkKey x y)
  (cond [(and (>= y 50) (< y 200))
         (cond [(and (>= x 950) (< x 1100)) stopKey] 
               [(and (>= x 800) (< x 950)) chooseFileKey] 
               [(and (>= x 350) (< x 500)) playloop] 
               [(and (>= x 500) (<= x 650)) stoploop]
               [else blankKey])]
...
;Used to put the key into the list of loops
(define (put x lon)
  (cons x (cons lon '())))
...
(define (handle-mouse ws x y event)
  (cond [(string=? event "button-down")
         (both (cond [(equal? (checkKey x y) playloop)
                      (both (make-ws "playloop" 0.0 0.5 0) (= FRAME-RATE 0))]
                     [else ws])
               (both (playKey (checkKey x y))
                     ws))]
        [(equal? (ws-keyLastPressed ws) "playloop")
         (record-loop x y)]
        [(equal? (ws-keyLastPressed ws) "1")
         (looper listloop)]
        [(string=? event "drag")
...
;records loop for four seconds
(define (record-loop x y)
  (cond
    [(<= FRAME-RATE (* 4 44100))
     (put (checkKey x y) listloop)]
    [(> FRAME-RATE (* 4 44100)) (make-ws "1" 0.0 0.5 0)]))
...
;list of the notes pressed after pressing the record looper button
(define listloop '())

;the world defintion takes a list
;list-> sound of the list
;plays the first sounds, then the second, and so forth until it is empty
(define (looper x)
  (cond [(empty? x) '()]
        [else (rs-append (play (first x)) (looper (rest x)))]))



;; if it's time, queue up the next section
;; of the song
;also try to play both tick and the looper for the tick-handler
(define (tick-fun ws)
;  (both (cond [(time-to-play? (ws-end-frame ws)
