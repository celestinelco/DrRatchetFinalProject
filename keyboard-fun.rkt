...
(define-struct ws [keyLastPressed slider-frac-x track-volume end-frame los lon])
...
; Returns note at correct start time in pstream given name and pstream
(define (create-note name pstream)
  (make-note (pstream-current-frame pstream) name))
...
(define (key-overlay lon BG)
   (cond [(empty? lon) BG]
                     [else (place-image (get-image (first lon)) 55 100
                                        (key-overlay (rest lon)))]))
...
(define (also pstream sound)
  (both (pstream-play pstream sound) (make-note pstream-current-frame sound))) 

;; Adds keys to a list
(define (keyAdd ws sound pstream)
  (both (cons (also pstream sound) (ws-lon ws)) ws))

(define quicksec 44100) 

;; Removes keys not playing from list
(define (keyRemove lon pstream)
  (cond
    [(empty? lon) '()]
    [else(cond [(> pstream-current-frame (+ (note-start (first lon)) quicksec)) (keyRemove (rest lon) pstream)]
